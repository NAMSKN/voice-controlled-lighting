...
import { Sun, Moon, Bed, Sofa, ChefHat, LampFloor, Mic, MicOff, Loader2, ClipboardList ,FileClock ,LogOut, User } from 'lucide-react';
...
const VirtualHome = () => {
  ...
  const [bulbStates, setBulbStates] = useState({
    kitchen: { intensity: 0 }, // 0: off, 1: warm, 2: bright
    hall: { intensity: 0 },
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      // Redirect to profile selection if no user data
      navigate('/');
    }
  }, [navigate]);
  ...
  // Add new log entry with user name
  const addLog = (action) => {
    const newLog = {
...
useEffect(() => {
  const fetchLogs = async () => {
    try {
      ...
      const response = await axios.get(`http://127.0.0.1:5000/conversation-logs/${userId}`);
 ...
 // Enhanced Microphone Button Component with Logs Toggle
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto bg-gray-100 min-h-screen relative">
     ...
     {/* User info remains on the right */}
